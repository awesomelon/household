...
import {
  ...
  calculateOneTimeCardPaymentDate,
...
export async function createTransactionDb(data: CreateTransactionDbPayload) {
  return prisma.$transaction(async (tx) => {
    const {
      ...
      workspaceId,
      createdById,
      adjustedDate, // 서비스 레이어에서 이미 처리된 날짜
    } = data;

    // 서비스 레이어에서 이미 처리된 날짜가 있으면 사용, 없으면 원래 날짜 사용
    const purchaseDate = adjustedDate || new Date(purchaseDateString);
...
interface UpdateTransactionDbPayload
  extends Omit<UpdateTransactionPayload, "useNextMonthForOneTime"> {
  workspaceId: string; // 권한 검증 및 데이터 격리
  adjustedDate?: Date; // 서비스 레이어에서 이미 처리된 날짜 (옵션)
}
...
export async function updateTransactionDb(
  ...
  const { workspaceId, adjustedDate, ...updatePayload } = data;
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (updatePayload.date) {
      // 서비스 레이어에서 이미 처리된 날짜가 있으면 사용, 없으면 원래 날짜 사용
      updateDataForTarget.date = adjustedDate || new Date(updatePayload.date);
    }
    ...
    if (newIsInstallment) {
      ...
      if (
        ...
        const effectivePurchaseDateForFee = updatePayload.date
          ? adjustedDate || new Date(updatePayload.date)
